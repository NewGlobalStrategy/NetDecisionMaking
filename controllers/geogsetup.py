...
#Unspecified Subdivision is already added - and not now adding this to every country
    ...
    subdivns = [[{'Country':'Canada (NA)'},['Alberta','British Columbia','Manitoba','New Brunswick',
            'Newfoundland and Labrador','Nova Scotia','Ontario','Prince Edward Island','Quebec',
            'Saskatchewan']],[{'Country':'United States (NA)'},['Alabama','Alaska','American Samoa', 
...
def countries():
    ...
    continents = ["Unspecified","Africa (AF)","Asia (AS)","Europe (EU)","North America (NA)","Oceania (OC)","South America (SA)"]
    for x in continents:
        if db(db.continent.continent_name == x).isempty():
            db.continent.insert(continent_name=x)
    
    countries=[[{'Continent':'Africa (AF)'},['Algeria (AF)','Angola (AF)','Benin (AF)','Botswana (AF)',
                'Burkina (AF)','Burundi (AF)','Cameroon (AF)','Cape Verde (AF)',
                'Central African Republic (AF)','Chad (AF)','Comoros (AF)','Congo (AF)',
                ...
                'Ivory Coast (AF)','Kenya (AF)','Lesotho (AF)','Liberia (AF)','Libya (AF)',
                ...
                'Brunei (AS)','Burma (Myanmar) (AS)','Cambodia (AS)',
                  'China (AS)','East Timor (AS)','India (AS)','Indonesia (AS)','Iran (AS)','Iraq (AS)',
                    'Israel (AS)','Japan (AS)','Jordan (AS)',
                  'Kazakhstan (AS)','Korea, North (AS)','Korea, South (AS)','Kuwait (AS)',
                'Kyrgyzstan (AS)','Laos (AS)','Lebanon (AS)','Malaysia (AS)',
                  'Maldives (AS)','Mongolia (AS)','Nepal (AS)','Oman (AS)','Pakistan (AS)',
                ...
                'Finland (EU)','France (EU)','Georgia (EU)','Germany (EU)','Greece (EU)',
                'Hungary (EU)','Iceland (EU)','Ireland (EU)','Italy (EU)',
                'Latvia (EU)','Liechtenstein (EU)','Lithuania (EU)','Luxembourg (EU)',
                ...
                'Cuba (NA)','Dominica (NA)','Dominican Republic (NA)','El Salvador (NA)',
                'Grenada (NA)','Guatemala (NA)','Haiti (NA)','Honduras (NA)',
                'Jamaica (NA)','Mexico (NA)','Nicaragua (NA)','Panama (NA)','Saint Kitts and Nevis (NA)',
                ...
                'New Zealand (OC)','Palau (OC)','Papua New Guinea (OC)','Samoa (OC)',
                ...
                'Guyana (SA)','Paraguay (SA)','Peru (SA)','Suriname (SA)','Uruguay (SA)','Venezuela (SA)']]]
